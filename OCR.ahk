OCR_Area() {
   ...
   selectionwidth := 160                                          ; Used for OCR() function
   selectionheight := 70                                          ; Used for OCR() function
   selectionretractionwidth := 60                                 ; Used for OCR() function
   selectionretractionheight := 30                                ; Used for OCR() function
   selectionstartx := mouseposx - selectionretractionwidth        ; Used for OCR() function
   selectionstarty := mouseposy - selectionretractionheight       ; Used for OCR() function
   ClipboardRec := ClipboardAll			                           ; Store current contents of clipboard into a variable to restore it afterwards
   Clipboard = 	                     					            ; Clear the clipboard
   MouseGetPos, mouseposx, mouseposy
   SystemCursor("Off")
   sleep 500
   hBitmap := HBitmapFromScreen(selectionstartx, selectionstarty, selectionwidth, selectionheight)
   pIRandomAccessStream := HBitmapToRandomAccessStream(hBitmap)
   DllCall("DeleteObject", "Ptr", hBitmap)
   OCR_Text := ocr(pIRandomAccessStream, "en")
   SystemCursor("On")
   ;msgbox, % OCR_Text
   Return OCR_Text
}

OCR_CleanFix() {                                            ; rlean & fix the OCR results
   OCR_Text := StrUnmark(OCR_Text)                                         ; Remove any accented characters in the copied results
   OCR_Text := StrReplace(OCR_Text, A_Space, "")
   RegExMatch(OCR_Text, "smi)(CAT|TGR|JGR)(...)", matchedpattern)
   OCR_Text := matchedpattern
   matchedpatternABC := substr(matchedpattern, 1, 3)               ; get the 3-letter tool code & save in variable
   matchedpattern123 := substr(matchedpattern, 4, 3)               ; get the 3-number tool code & save in variable
   matchedpatternABC := RegExReplace(matchedpatternABC, "smi)(e)(..)", "T$2")   ; Replace E for T if found E** (ie EGR)
   matchedpatternABC := RegExReplace(matchedpatternABC, "smi)(i)(..)", "T$2")   ; Replace I for T if found I** (ie IGR)
   matchedpatternABC := RegExReplace(matchedpatternABC, "smi)(..)(P)", "$1R")   ; Replace P for R if found **P (ie TGP)
   matchedpatternABC := StrReplace(matchedpatternABC, "4", A)
   matchedpattern123 := StrReplace(matchedpattern123, "o", 0)
   matchedpattern123 := StrReplace(matchedpattern123, "i", 1)
   matchedpattern123 := StrReplace(matchedpattern123, "a", 4)
   matchedpattern := matchedpatternABC matchedpattern123
   OCR_Text := matchedpattern
   return OCR_Text
}

OCR_Match() {
   If InStr(toolname_list, OCR_Text) && (OCR_Text != "")                         ;check that captured word exists in toolname_list and isn't blank
      Return 1                                                       ; match
   else
      return 0                                                       ; no match
   }
   Show_GuiTip(hwnd_num, OffX, OffY, GuiTipW, GuiTipH) {                                                   ; -----CREATE & SHOW GUITIP-----
   Global
   OffsetX := OffX                                    ; pass OffX/OffY as global OffsetX/OffsetY for use in FOLLOMOUSE
   OffsetY := OffY
   OffsetY%hwnd_num% := OffsetY
   Gui, PGui:+AlwaysOnTop +ToolWindow -SysMenu -Caption +LastFound +hwndhGuiTip%hwnd_num%
   WinSet, ExStyle, -0x20                         				         ; mouse-click transparency?
   WinSet, Transparent, 220
   Gui, PGui:Color, 0x00aceb                        	                     ; Gui background color
   Gui, PGui:Font, cWhite s10 wBold, Segoe UI                             ; use c0x58b530 for TEL green color
   hdr := substr(OCR_Text, 1, 3)
   Gui, PGui:Add, Text,, % %OCR_Text%_array[2] "`n" header_%hdr%_array[3] ": " %OCR_Text%_array[3]
   ; ToolTip, % %OCR_Text%_array[2] "`n" %OCR_Text%_array[3] "`n`nClose: {ESC}`nTEL Info Tool: {CTRL}{CTRL}"
   Gui, PGui:Font, cblack s8 wRegular
   MouseGetPos, mouseX, mouseY
   Gui, PGui:Show, % "x" mouseX + OffsetX  " y" mouseY-GuiTipH-OffsetY " w" GuiTipW " h" GuiTipH      ; move the GuiTip window
   ;if A_ScreenDPI = 96                               		                           ; standard screen DPI
   WinSet, Region, % 0 "-" 0 " w" GuiTipW " h" GuiTipH " R" 10 "-" 10  ; rounded corners
}
...
LowLevelMouseProc(nCode, wParam, lParam) {
   ...
   if (nCode = "timer") {
      while coords[1] {
         ...
         try Gui, Show, x%x% y%y% w%w% h%h% NA
      }
   }
   else {
...
ocr(file, lang := "en") {
   ...
   if (file = "ShowAvailableLanguages")
   {
      ...
      loop % count
      {
         ...
         if (bool = 1)
         {
            ...
            OCR_text .= StrGet(buffer, "UTF-16") "`n" ;MIGHT HAVE WRONGLY CHANGED TEXT TO OCR_TEXT
         }
   ...
   loop % count
   {
      ...
      buffer := DllCall("Combase.dll\WindowsGetStringRawBuffer", "ptr", hText, "uint*", length, "ptr")
      OCR_Text .= StrGet(buffer, "UTF-16") "`n"
      ObjRelease(OcrLine)
   }
...
StrUnmark(string) {                                            ; removes diacritrics from characters.
	...
	return	RegExReplace(StrGet(&buf, len, "UTF-16"), "\pM") ; Remove combining marks and return result.
}